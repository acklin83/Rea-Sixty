...
- **Enabling at runtime:** three switches under *Settings → FX Learn → Modifier layers* — *"Hold Option for the FX-Learn Option layer"*, *"Hold Control for the FX-Learn Control layer"* and *"Hold Control+Option for the combined FX-Learn layer"*. All three are on by default. Holding **Option/Alt** selects the Option layer, **Control** the Control layer, and **both together** the Control+Option layer; with the combined switch off, both-held falls back to Normal, and holding neither is always Normal. The held modifier takes effect live — on the bound control, the UC1 LCD readout, and the Learn-HUD's layer badge. *(Windows: all three layers are reachable with the left hand alone, so the other stays on the surface. **Left Alt** is the Option layer, **left Control** the Control layer, and the two together, side by side in the bottom row, the combined one. **AltGr** (the right Alt on European layouts) reaches the Option layer as well, even though Windows reports it as Ctrl+Alt: the synthetic Ctrl is dropped so AltGr can never be mistaken for the combined layer. Any Alt-and-Control pairing other than the two left keys gives the Option layer on its own.)*
