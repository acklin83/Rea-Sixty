...
| Bank names follow Shift | A bank has two full sets, Plain and Shift, and each can carry its own name. On, the readouts show the set you are **holding**, so Shift announces that bank's Shift set. Off, they stay on the Plain name however long you hold, which is what you want if you work with Shift down. Applies to the banner and to the panel's bank menu alike. On by default. |
