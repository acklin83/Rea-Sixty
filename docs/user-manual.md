...
This manual documents Rea-Sixty v0.6.0. Earlier manuals (anything dated before 2026-09-06) are superseded.
...
sits on `Shift` + `5-8`, which is where it ships: that key has no job of its own in
this view. **UF1 Presets** (*Hardware Modes*) is in the picker as well, for a key of
your choosing. While it is open, **SK3** is *Navigate Back* and **V4** scrolls, with a
push to load.
...
**Parameter Groups** — the eight keys are the eight groups, labelled with your group names (falling back to `Grp 1`…`Grp 8`). A key is bright when the focused track belongs to that group. What each gesture does is yours, in the bank's own settings: five rows, Push, +Shift, +Cmd, +Ctrl and Long-press, each choosing between *Join / leave the group*, *Group on / off* and nothing. Push joins and long-press arms out of the box, which is what the bank did before the rows existed. Joining and leaving applies to every selected track. The choice is global, like the FX bank's gestures, so one setting drives every Parameter-Groups bank on every surface. Group names and each group's key colour are edited in the separate **Parameter Groups** Settings section, not here. A group left white keeps the key colour it always had; give it one and the bank's key wears it, bright for a member and dim for the rest. The colour is global, like the track-colour bank's palette: names and the on/off flag belong to the project, but what a group looks like on the surface is the same wherever you open it. Membership, the group names and their on/off state are all stored in the project. Only the multi-select toggle is global.
...
You build a step cycle **in the editor**: in the FX-Learn V-Pot right-click menu (or the on-screen Learn-HUD), pick **Step cycle (turn)** and edit the list — drag to reorder, untick to exclude an option, **+ Add step** to add one by name. The same editor opens for UC1 pots. Re-learning a pot the normal way clears any step cycle on it, so a continuous pot is one re-learn away.

*Building one by touch — wiggling the control with Touch-to-Learn armed and then clicking the plug-in's buttons — is switched off on both surfaces: it wrote step cycles nobody asked for.*
...
- **Pair** — press it, then touch a second parameter on that same track. It joins the same pot. The surface has the same thing as *Sticky Pot: Pair next touched Parameter*, which takes whichever pinned track you touch. Both values are noted at that moment, so pairing itself moves nothing; only the next turn of the pot does. A parameter on another track is ignored, so grabbing the wrong plug-in first costs nothing. The capture gives up after twenty seconds, like the pin capture.
- **Ratio** — how far the second parameter travels for a given move of the first, and in which direction. `-1.00` is counter-running one to one, which is what an 1176 wants: input up, output down, level unchanged. A positive value runs both the same way, a smaller number moves the second one less. The second parameter stops at its own end of travel while the pot keeps going. Changing the ratio pivots around where the pair stands at that moment, so nothing jumps.

**Move either parameter by hand** — in the plug-in, from another controller — and the pot takes over from there. The ratio stays as it is; what moves is the point it counts from. Your correction is the new zero, not something the next detent undoes. The same applies after a project load: whatever the two are at when you first touch the pot is where the coupling starts.
...
- **Reset to factory defaults** — confirmation popup. Replaces bindings + learned FX + every Settings preference with the baked-in factory configuration. Per-project Selection Sets and Parameter Group memberships are untouched.
...
- **Where the devices go afterwards.** Our INF puts them in class `USBDevice`, so in Device Manager they move out of *Universal Serial Bus controllers* into *Universal Serial Bus devices*, and each is renamed for itself: `SSL UF8 (Rea-Sixty / WinUSB)`, `SSL UC1 (Rea-Sixty / WinUSB)`, `SSL UF1 (Rea-Sixty / WinUSB)`. Looking for the old name in the old category and finding nothing does not mean the unit is dead.
...
- **Sticky Pot: Pair next touched Parameter** — arm the pairing. The next plug-in parameter you touch joins the pin on its own track, so one pot moves both, counter-running until you change the ratio in *Settings, Sticky Pot*. A touch on a track that has no pin is ignored and the arm keeps waiting. Lit while armed.
...
A pin can carry a **second parameter**, so one pot moves two. Arm it from the surface with *Sticky Pot: Pair next touched Parameter*, or in *Settings, Sticky Pot* with **Pair** on that pin's row: either way you then touch the second parameter on the same track, and from then on it follows the pinned one at the ratio you give it. `-1.00`, the default, runs it exactly counter to the pin, which is how you hold an 1176 at one level while you drive it harder. Both values are anchored when you pair them, so nothing jumps at that moment. The surface keeps showing the first parameter, and the second stays an ordinary parameter you can still reach in the plug-in or from anywhere else: it only moves when the pot moves the pin, and if you have moved it yourself in the meantime, the pot carries on from where you left it.
...
- Polarity (if mapped) → action of choice *(UC1 only — the UF8 half offers V-Pot push, Cut and Solo)*
...
If TotalReaper isn't installed the keys keep their normal behaviour, because a missing action falls through to the old solo and mute. **The gain knob is the exception**: with the gain toggle on, the rotation is consumed and sent to an action that is not there, so the pot moves nothing at all rather than falling back to pan.
...
3. **Refresh lights + scenes** re-reads everything. Rea-Sixty does that by itself every second while it is connected, and re-reads rooms and zones on every tenth pass, so you only need this after adding a lamp.
...
The colour bar shows what the lamp is actually showing. When the lamp is off the bar falls back to the configured **Bar** colour rather than keeping the lamp's last one.
...
Storage: each track's member-of-which-slots state is a bitmask in P_EXT (`P_EXT:reasixty:pg_mask`). Slot names and active flags ride in the project file; the global JSON holds only the multi-select toggle.
...
A button carries four short-press slots, four long-press and four double-press, one per modifier — twelve in all, and none of them a combination.
...
Settings → Modes → AUTO → "Show only tracks armed for automation writing (hide Trim / Read)" — within AUTO selection mode only, hides tracks whose automation mode is Off/Trim (0) or Read (1).
