...
void seedFactoryDefaults_(Config& c)
{
    ...
    // ⇨ PAN IS PAN AGAIN, since 2026-09-02. It carried the Nav-Mode toggle from
    // Phase 2.8 onward, for the good reason that nothing else did — the NAV key
    // beside the encoder shipped unbound. Now that NAV carries it, two keys were
    // doing the same thing and the one with PAN printed on it was the wrong one
...
// Per-set user name, by set number. Flat list, only non-empty entries written.
void serializeSoftKeySetNames_(const Config& c, std::ostringstream& os)
{
    bool any = false;
    for (int n = 1; n <= kSoftKeySetCount && !any; ++n) {
        int l = 0, q = 0;
        if (softKeySetToLayerQuick(n, l, q) && !c.userQuicks[l].quicks[q].name.empty())
            any = true;
    }
    if (!any) return;
    os << ",\n  \"softkey_set_names\": [";
    bool first = true;
    for (int n = 1; n <= kSoftKeySetCount; ++n) {
        int l = 0, q = 0;
        if (!softKeySetToLayerQuick(n, l, q)) continue;
        const std::string& nm = c.userQuicks[l].quicks[q].name;
...
void parseSoftKeySetNames_(wdl_json_element* root, Config& out)
{
    auto* arr = root->get_item_by_name("softkey_set_names");
    if (!arr || !arr->is_array() || !arr->m_array) return;
    const int n = arr->m_array->GetSize();
    for (int i = 0; i < n; ++i) {
        wdl_json_element* eo = arr->enum_item(i);
        if (!eo || !eo->is_object()) continue;
        int setNo = -1;
        if (auto* v = eo->get_item_by_name("set"))
            if (auto* t = v->get_string_value(true)) setNo = std::atoi(t);
        int l = 0, q = 0;
        if (!softKeySetToLayerQuick(setNo, l, q)) continue;
        if (auto* v = eo->get_item_by_name("name"))
            if (auto* t = v->get_string_value(true))
                out.userQuicks[l].quicks[q].name = t;
    }
}
...
// v42 (2026-09-23): a long-press slot can repeat while the key is held
// ("repeat" / "repeat_ms" per slot). Additive: a file without them repeats
// nothing, which is what every binding did before. (On main the same step is
// v48; the six numbers between are RME side-car seeds that are not on this
// branch.)
...
// ⚠ THERE WAS A SECOND PIN HERE, g_bankModifierPinMenu, for the focused panel's
// bank menu, with an arbitration between the two. It went in on 2026-09-01 as a
// separate store — the editor republishes ITS answer on every frame the window
// draws, including -1, so a menu pin sharing this store was wiped within a frame
// of being set. Both the menu's Shift rows and the pin came out again on
// 2026-09-02, so the arbitration had nothing left to arbitrate.
// If a second writer to this ever comes back, it needs its own store for the
...
{
    int l = 0, q = 0;
    if (!softKeySetToLayerQuick(setNo, l, q)) return;
    std::lock_guard<std::mutex> lk(g_cfgMutex);
    g_cfg.userQuicks[l].quicks[q].name = name;
    persistLocked_();
}
...
// Default soft-key label per builtin — the name that fills the Label field
// when an action is assigned and the user has not typed one. Twelve characters,
// which is the surface's cap; a family shares a prefix (Enc / UF1 / UF8 / Hue /
// Zoom) so eight of them side by side stay readable. The jog cross is the one
// family without one: there the OBJECT is the information (Item / Point / Razor
// / Fade) and a prefix would not leave room for it. Frank curated the list,
// 2026-09-02.
//
// ⚠ EVERY NEW BUILTIN NEEDS A LINE HERE. Without one the Label field stays
// empty and the key ships nameless, which is what this table exists to stop.
struct BuiltinLabel { const char* name; const char* label; };
static const BuiltinLabel kBuiltinLabels[] = {
    { "auto_latch", "Latch" },
    { "auto_latch_global", "Latch All" },
    { "auto_latch_prv", "Latch Prv" },
    { "auto_latch_prv_global", "LatchPrv All" },
    { "auto_off", "Off" },
    { "auto_off_global", "Off All" },
    { "auto_read", "Read" },
    { "auto_read_global", "Read All" },
    { "auto_touch", "Touch" },
    { "auto_touch_global", "Touch All" },
    { "auto_trim", "Trim" },
    { "auto_trim_global", "Trim All" },
    { "auto_write", "Write" },
    { "auto_write_global", "Write All" },
    { "automation_zero_all", "Auto Clear" },
    { "bank_by_1_left", "Bank <1" },
    { "bank_by_1_right", "Bank >1" },
    { "bank_left", "Bank <" },
    { "bank_right", "Bank >" },
    { "bc_copy_own_toggle", "BC Copy/Own" },
    { "bc_cycle", "BC Cycle" },
    { "bc_track_scroll", "BC Scroll" },
    { "bc_track_scroll_select", "BC Scrl Sel" },
    { "brightness_both_down", "Bright -" },
    { "brightness_both_up", "Bright +" },
    { "brightness_lcds_down", "LCD -" },
    { "brightness_lcds_up", "LCD +" },
    { "brightness_leds_down", "LED -" },
    { "brightness_leds_up", "LED +" },
    { "close_all_fx_guis", "Close All" },
    { "copy_bc_1", "Copy BC 1" },
    { "copy_bc_2", "Copy BC 2" },
    { "copy_bc_3", "Copy BC 3" },
    { "copy_bc_4", "Copy BC 4" },
    ...
    { "copy_cs_7", "Copy CS 7" },
    { "copy_cs_8", "Copy CS 8" },
    { "copy_fav_1", "Copy Fav 1" },
    { "copy_fav_2", "Copy Fav 2" },
    { "copy_fav_3", "Copy Fav 3" },
    { "copy_fav_4", "Copy Fav 4" },
    { "copy_fav_5", "Copy Fav 5" },
    { "copy_fav_6", "Copy Fav 6" },
    { "copy_fav_7", "Copy Fav 7" },
    { "copy_fav_8", "Copy Fav 8" },
    { "cs_copy_own_toggle", "CS Copy/Own" },
    { "cs_cycle", "CS Cycle" },
    { "domain_bc", "SSL BC Row" },
    { "domain_cs", "SSL CS Row" },
    { "encoder_bank_by_1", "Enc Bank 1" },
    { "encoder_bc_cycle", "Enc BC Cyc" },
    { "encoder_cs_cycle", "Enc CS Cyc" },
    { "encoder_fav_cycle", "Enc Fav Cyc" },
    { "encoder_focus", "Enc Wheel" },
    { "encoder_fx_cycle", "Enc FX Cyc" },
    { "encoder_fx_move", "Enc FX Move" },
    { "encoder_fx_scroll_all", "Enc FXScroll" },
    { "encoder_instance", "Enc Instance" },
    { "encoder_instance_scroll_all", "Enc InstScrl" },
    ...
    { "fx_param_dec", "Param -" },
    { "fx_param_inc", "Param +" },
    { "fx_scroll_all", "FX Scroll" },
    { "home", "Home (clear)" },
    { "hue_all_off", "Hue Off" },
    { "hue_rec_light", "Hue RecLight" },
    { "hue_scene_recall", "Hue Scene" },
    { "hue_scene_recall_dynamic", "Hue ScnDyn" },
    { "instance_cycle", "Inst Cycle" },
    ...
    { "jog_env_point_next_add", "Point > Add" },
    { "jog_env_point_prev", "Point <" },
    { "jog_env_point_prev_add", "Point < Add" },
    { "jog_env_target_toggle", "Env Target" },
    ...
    { "jog_fade_right", "Fade >" },
    { "jog_fade_right_add", "Fade > Add" },
    { "jog_fade_up", "Fade Up" },
    { "jog_fade_up_add", "FadeUp Add" },
    { "jog_item_next", "Item >" },
    { "jog_item_next_add", "Item > Add" },
    { "jog_item_prev", "Item <" },
    { "jog_item_prev_add", "Item < Add" },
    { "jog_item_track_down", "Item Down" },
    { "jog_item_track_down_add", "ItemDwn Add" },
    { "jog_item_track_up", "Item Up" },
    { "jog_item_track_up_add", "ItemUp Add" },
    ...
    { "jog_mode_scrub", "Jog Scrub" },
    { "jog_nav_center", "Jog Centre" },
    { "jog_nav_down", "Jog Down" },
    { "jog_nav_left", "Jog Left" },
    { "jog_nav_right", "Jog Right" },
    { "jog_nav_up", "Jog Up" },
    { "jog_razor_bottom", "Razor Btm" },
    { "jog_razor_left", "Razor <" },
    { "jog_razor_right", "Razor >" },
    { "jog_razor_top", "Razor Top" },
    { "jog_razor_whole", "Razor All" },
    { "jog_zoom_selection", "Zoom Sel" },
    ...
    { "mod_ctrl", "Ctrl" },
    { "mod_shift", "Shift" },
    { "mode_banner_toggle", "Mode Banner" },
    { "mouse_scroll", "Mousewheel" },
    { "multi_select_as_temp_group_toggle", "Multi Group" },
    { "page_left", "Page <" },
    { "page_right", "Page >" },
    { "pan_force", "V-Pots Pan" },
    { "param_group_add_1", "P-Grp 1 Add" },
    { "param_group_add_2", "P-Grp 2 Add" },
    { "param_group_add_3", "P-Grp 3 Add" },
    { "param_group_add_4", "P-Grp 4 Add" },
    { "param_group_add_5", "P-Grp 5 Add" },
    { "param_group_add_6", "P-Grp 6 Add" },
    { "param_group_add_7", "P-Grp 7 Add" },
    { "param_group_add_8", "P-Grp 8 Add" },
    { "param_group_clear_1", "P-Grp 1 Clr" },
    { "param_group_clear_2", "P-Grp 2 Clr" },
    { "param_group_clear_3", "P-Grp 3 Clr" },
    { "param_group_clear_4", "P-Grp 4 Clr" },
    { "param_group_clear_5", "P-Grp 5 Clr" },
    { "param_group_clear_6", "P-Grp 6 Clr" },
    { "param_group_clear_7", "P-Grp 7 Clr" },
    { "param_group_clear_8", "P-Grp 8 Clr" },
    { "param_group_remove_all", "P-Grp RemAll" },
    { "param_group_toggle_1", "ParamGrp 1" },
    { "param_group_toggle_2", "ParamGrp 2" },
    { "param_group_toggle_3", "ParamGrp 3" },
    { "param_group_toggle_4", "ParamGrp 4" },
    { "param_group_toggle_5", "ParamGrp 5" },
    { "param_group_toggle_6", "ParamGrp 6" },
    { "param_group_toggle_7", "ParamGrp 7" },
    { "param_group_toggle_8", "ParamGrp 8" },
    { "playhead_nudge", "Nudge Cursor" },
    { "plugin_bypass", "FX Bypass" },
    { "plugin_move_down", "FX Down" },
    { "plugin_move_up", "FX Up" },
    { "plugin_offline", "FX Offline" },
    ...
    { "recv_all_4", "Recv 4 All" },
    { "recv_all_5", "Recv 5 All" },
    { "recv_all_6", "Recv 6 All" },
    { "recv_all_7", "Recv 7 All" },
    { "recv_all_8", "Recv 8 All" },
    { "recv_this", "Track Recvs" },
    { "restart", "Restart Ext" },
    { "select_relative", "Sel Step" },
    { "selection_clear_all", "Clear Sel" },
    { "selection_mode_auto", "Automation" },
    { "selection_mode_dynamount", "DynaMount" },
    { "selection_mode_hue", "Hue Mode" },
    { "selection_mode_instance", "FX Cycle" },
    { "selection_mode_instance_cycle", "Inst Cycle" },
    { "selection_mode_norm", "Select" },
    { "selection_mode_rec", "Rec Arm" },
    { "selection_mode_rec_mon", "Rec + Mon" },
    { "selset_cycle", "Cycle Sets" },
    { "selset_recall", "Recall Set" },
    { "selset_save", "Save Selset" },
    { "send_all_1", "Send 1 All" },
    { "send_all_2", "Send 2 All" },
    { "send_all_3", "Send 3 All" },
    { "send_all_4", "Send 4 All" },
    { "send_all_5", "Send 5 All" },
    { "send_all_6", "Send 6 All" },
    ...
    { "softkey_bank_1", "Set 8" },
    { "softkey_bank_2", "Set 9" },
    { "softkey_bank_3", "Set 1" },
    { "softkey_bank_4", "Set 2" },
    { "softkey_bank_5", "Set 3" },
    { "softkey_bank_6", "Set 4" },
    { "softkey_bank_7", "Set 5" },
    { "softkey_bank_8", "Set 6" },
    { "softkey_bank_9", "Set 7" },
    { "softkey_bank_select", "Bank Sel" },
    { "softkey_set_engage", "Soft-Key Set" },
    { "ssl_bank_1", "SSL Row 1" },
    { "ssl_bank_2", "SSL Row 2" },
    { "ssl_bank_3", "SSL Row 3" },
    { "ssl_bank_4", "SSL Row 4" },
    { "ssl_bank_5", "SSL Row 5" },
    { "ssl_bank_vpot", "SSL V-POT" },
    ...
    { "uf1_extender_side", "UF1 Ext Side" },
    { "uf1_fine_toggle", "UF1 Fine" },
    { "uf1_five_to_eight", "UF1 5-8" },
    { "uf1_flip", "UF1 Flip" },
    { "uf1_hue", "UF1 Hue" },
    { "uf1_master", "UF1 Master" },
    { "uf1_page_step", "UF1 Page" },
    { "uf1_presets", "UF1 Presets" },
    { "uf1_sends_receives_toggle", "UF1 Snd/Rcv" },
    { "uf1_strip_mode_toggle", "UF1 Strip" },
    { "uf1_strip_mode_toggle_with_gui", "UF1 Strp+GUI" },
    { "uf1_time_display_step", "UF1 Time" },
    { "uf1_view_cycle", "UF1 View" },
    { "uf1_view_daw", "UF1 DAW" },
    { "uf1_view_meter", "UF1 Meter" },
    { "uf1_view_plugin", "UF1 Plugin" },
    { "uf1_view_sends", "UF1 Sends" },
    { "uf1_vpot_reset", "UF1 V-Pot" },
    { "uf8_plugin_mode_toggle", "UF8 Plug-in" },
    { "uf8_plugin_mode_toggle_with_gui", "UF8 Plg+GUI" },
...
const char* builtinShortLabel(const std::string& n)
{
    for (const auto& e : kBuiltinLabels)
        if (n == e.name) return e.label;
    return "";
}
