...
// Host-OS modifier read for Keyboard-macro capture. ReaImGui receives mac
// modifiers already SWELL-swapped (⌘ arrives as VK_CONTROL, ⌃ as VK_LWIN —
// swell-kb.mm:153/256), so ImGui's LeftCtrl/LeftSuper report Cmd/Ctrl swapped.
// Read the real flags from the OS instead, exactly like main.cpp's host*Held_.
...
// 2026-06-26 — cs flag selects the Channel-Strip or Bus-Comp library).
...
// Same matcher, handed to MixerWindow's rail search box. Declared in
// SettingsScreen.h — a thin forwarder beats a second copy of the tokeniser.
// Frank 2026-08-09.
std::vector<std::string> settingsSearchTokens(const char* query)
{
    return searchTokensLower_(query);
}
...
// Only one capture is armed at a time across the whole editor, keyed by the
// step's unique prefix. While armed, the next non-modifier key press (read from
// the focused Settings window) is combined with the held modifiers into a
// neutral chord string. ESC cancels. A manual text field stays available as a
// fallback for hand-tweaks or combos the window would otherwise swallow.
static std::string g_kbCapturePrefix;

// Neutral name of the first non-modifier ImGui key pressed this frame, else
// nullptr. Repeats are ignored so a held key captures once.
static const char* capturedKeyName_(ImGui_Context* ctx)
{
    struct K { int key; const char* name; };
    const K keys[] = {
        {ImGui_Key_A,"a"},{ImGui_Key_B,"b"},{ImGui_Key_C,"c"},{ImGui_Key_D,"d"},
        {ImGui_Key_E,"e"},{ImGui_Key_F,"f"},{ImGui_Key_G,"g"},{ImGui_Key_H,"h"},
        {ImGui_Key_I,"i"},{ImGui_Key_J,"j"},{ImGui_Key_K,"k"},{ImGui_Key_L,"l"},
        {ImGui_Key_M,"m"},{ImGui_Key_N,"n"},{ImGui_Key_O,"o"},{ImGui_Key_P,"p"},
        {ImGui_Key_Q,"q"},{ImGui_Key_R,"r"},{ImGui_Key_S,"s"},{ImGui_Key_T,"t"},
        {ImGui_Key_U,"u"},{ImGui_Key_V,"v"},{ImGui_Key_W,"w"},{ImGui_Key_X,"x"},
        {ImGui_Key_Y,"y"},{ImGui_Key_Z,"z"},
        {ImGui_Key_0,"0"},{ImGui_Key_1,"1"},{ImGui_Key_2,"2"},{ImGui_Key_3,"3"},
        {ImGui_Key_4,"4"},{ImGui_Key_5,"5"},{ImGui_Key_6,"6"},{ImGui_Key_7,"7"},
        {ImGui_Key_8,"8"},{ImGui_Key_9,"9"},
        {ImGui_Key_F1,"f1"},{ImGui_Key_F2,"f2"},{ImGui_Key_F3,"f3"},{ImGui_Key_F4,"f4"},
        ...
        {ImGui_Key_LeftArrow,"left"},{ImGui_Key_RightArrow,"right"},
        {ImGui_Key_UpArrow,"up"},{ImGui_Key_DownArrow,"down"},
    };
                      ...
                      bool isDoublePress = false)
{
    ...
    // ---- Keyboard macro ----
    // Synthesises a key chord to REAPER via kbd_translateAccelerator (in-
    // process, never OS-global). Multi-key macros are built as a chain of
    // Keyboard steps with a wait_ms between each — use "+ Add step" below to
    // append entries, each its own captured chord with a delay. Frank 2026-06-29.
    sectionRadio("rd_kbd", "Keyboard macro", ActionType::Keyboard);
    if (*f.type == ActionType::Keyboard) {
        ImGui_Indent(ctx, /*indent_w*/ nullptr);
        if (drawKeyboardChordField(ctx, prefix, *f.action))
            dirty = true;
        ImGui_Unindent(ctx, /*indent_w*/ nullptr);
    }
...
void drawBindingEditor(ImGui_Context* ctx, int layer, ButtonId id)
{
    ...
    }   // end !idIsUc1 && !idNoLed && !idIsUf1NoLed — LED block hidden for UC1 + UF8/UF1 encoders + pushes
...
// (linkIdx) changes. g_fxlAddParam is the chosen plugin param (-1 = none
// yet); g_fxlAddNorm the value for non-stepped params; g_fxlAddFilter the
// param-list search text.
int   g_fxlAddLinkIdx = -1;
int   g_fxlAddParam   = -1;
float g_fxlAddNorm    = 0.5f;
char  g_fxlAddFilter[64] = {};
...
// ---- Per-button push-cycle steps -----------------------------------------
//
// A button's push-cycle is an ordered list of (param, value) steps. Empty +
// a bound param = the shipped auto-cycle-all-options behaviour; a non-empty
// list overrides it with a curated subset of one param's options OR a
// multi-param macro. See UserLinkSlot::pushSteps + the UC1Surface runtime.

// Current push-cycle for a slot (empty when the slot has none / doesn't
// exist). Returns a copy for safe rendering.
std::vector<uf8::PushStep> slotPushSteps_(int linkIdx)
{
    if (g_editingMatch.empty() || linkIdx < 0) return {};
    for (const auto& m : uf8::user_plugins::get().maps) {
        if (m.match != g_editingMatch) continue;
...
// True when a slot exists for this linkIdx and carries a real bound param
// (vst3Param >= 0). Distinguishes a curated-subset slot (keep the param,
// auto-cycle restored when steps cleared) from a macro slot (drop entirely
// when steps cleared).
bool slotHasBoundParam_(int linkIdx)
{
...
// Like mutateSlot_ but creates a macro slot (vst3Param = -1) when none
// exists, so push-cycle steps can attach to an otherwise-unmapped button.
template <class Mut>
void mutateOrCreateSlot_(int linkIdx, Mut&& mut)
{
    if (g_editingMatch.empty() || linkIdx < 0) return;
    auto cat = uf8::user_plugins::get();
    for (auto& m : cat.maps) {
        ...
        for (auto& s : m.slots)
            if (s.linkIdx == linkIdx) { slot = &s; break; }
        if (!slot) {
            uf8::UserLinkSlot ns{};
            ns.linkIdx   = linkIdx;
            ns.vst3Param = -1;   // macro slot — no primary param
...
// Single writer for the push-cycle. An empty list drops a macro slot
// entirely (button back to unmapped) but only clears the list on a param-
// bound slot (auto-cycle restored). All add/remove/reorder edits route a
// freshly-built vector through here.
void setSlotPushSteps_(int linkIdx, std::vector<uf8::PushStep> steps)
{
    if (steps.empty() && !slotHasBoundParam_(linkIdx)) {
        unbindSlot_(linkIdx);
        return;
    }
...
void drawFxLearnCurveEditorPopup_(ImGui_Context* ctx)
{
    ...
    if (g_curveEditorTarget.supportsSensitivity) {
        // Plain typeable value box — same style as Settings → Devices →
        // V-Pot / encoder feel (resolution). No slider; range 0.01..4.0 so a
                     ...
                     float scale)
{
    ...
    // Push-cycle applies to the discrete channel-strip buttons (EQ Type,
    // HF/LF Bell, Fast Attack, Peak, Expand, Gate Attack, …) — any button-
    // kind control except the plug-in-bypass IN toggles (linkIdx 0). A
    // macro slot has steps but no bound param, so track that separately
    // from isMapped for the ring colour + popup gate.
    ...
    if (exists && (isMapped || isStepCycleCtrl)) {
        ...
        if (ImGui_BeginPopupContextItem(ctx, popId, nullptr)) {
            ...
            // ---- Push cycle (discrete channel-strip buttons) -------------
            // Curate which option values a button steps through on push: a
            ...
            if (isStepCycleCtrl) {
                ...
                if (g_fxlAddLinkIdx != ctrl.linkIdx) {
                    g_fxlAddLinkIdx  = ctrl.linkIdx;
                    g_fxlAddParam    = -1;
                    g_fxlAddNorm     = 0.5f;
                    g_fxlAddFilter[0] = '\0';
                }

                // Discrete-options probe: option count (>=2) for a param, or
                // 0 when it has none (continuous → useless in a cycle, the
                // value would just sit there). Uses live FX step sizes; falls
                // back to the snapshot's wasEnum flag when no instance is
                // loaded. `pStepOut` is the normalised step between options.
                auto optionCountFor = [&](int p, double& pStepOut) -> int {
                    pStepOut = 1.0;
                    if (fx.ok) {
                        double s=0.0, a=0.0, b=0.0; bool isT=false;
                ...
                // The bound param's discrete options (if any). An empty
                // pushSteps + a discrete bound param shows ALL options as a
                // checkbox list (the "untick to exclude" view) — excluded
                // options stay in place, no separate layout to flip to.
                double boundStep = 1.0;
                const int boundOpts =
                    isMapped ? optionCountFor(mapped, boundStep) : 0;
                const bool boundDiscrete = boundOpts >= 2;

                // Effective ordered list. When nothing is stored yet but the
                // bound param is discrete, synthesise all options (enabled,
                // natural order) so they're visible + reorderable from the
                // start; the first edit materialises this into pushSteps.
                std::vector<uf8::PushStep> view = slotPushSteps_(ctrl.linkIdx);
                if (view.empty() && boundDiscrete) {
                    for (int i = 0; i < boundOpts; ++i) {
                        float nrm = static_cast<float>(i * boundStep);
                        if (nrm > 1.0f) nrm = 1.0f;
                ...
                // Persist an edited list. If it's exactly the bound param's
                // options, all enabled in natural order, revert to auto-cycle
                // (store nothing) so "all on" stays the lightweight default.
                auto commit = [&](std::vector<uf8::PushStep> v) {
                    bool canonical = boundDiscrete
                        && static_cast<int>(v.size()) == boundOpts;
                    if (canonical) {
                        for (int i = 0; i < static_cast<int>(v.size()); ++i) {
                            float nrm = static_cast<float>(i * boundStep);
                            if (nrm > 1.0f) nrm = 1.0f;
                            const auto& s = v[i];
                            const float dn = s.norm > nrm ? s.norm - nrm
                                                          : nrm - s.norm;
                            if (s.vst3Param != mapped || !s.enabled
                                || dn > 1e-4f) { canonical = false; break; }
                        }
                    }
                    setSlotPushSteps_(ctrl.linkIdx,
                        canonical ? std::vector<uf8::PushStep>{} : v);
                };

                // Unified list: every entry stays visible. The ≡ handle drags
                // to reorder; the checkbox includes/excludes the entry from
                // the cycle (excluded ones keep their place); a macro step (a
                // param other than the bound one) also gets an x to remove it.
                if (!view.empty()) {
                    ImGui_TextDisabled(ctx,
                        "Drag \xE2\x89\xA1 to reorder \xC2\xB7 untick to exclude:");
                    int dragFrom = -1, dragTo = -1, removeIdx = -1;
                    for (int i = 0; i < static_cast<int>(view.size()); ++i) {
                        const uf8::PushStep& st = view[i];
                        const bool foreign = !isMapped || st.vst3Param != mapped;
                        char pn[96] = {0};
                        if (foreign && editing)
                            paramNameFor_(*editing, fx, st.vst3Param,
                                          pn, sizeof(pn));
                        char vb[64] = {0};
                        if (fx.ok)
                            TrackFX_FormatParamValueNormalized(fx.tr, fx.fxIdx,
                        ...
                        if (foreign)
                            ...
                            snprintf(disp, sizeof(disp), "%s",
                                     vb[0] ? vb : "(option)");
                        char cbId[208];
                        snprintf(cbId, sizeof(cbId), "%s##fxl_ps_cb_%d", disp, i);

                        // ≡ drag handle — source + drop target for reordering.
                        char hId[40];
                        snprintf(hId, sizeof(hId),
                                 "\xE2\x89\xA1##fxl_ps_h_%d", i);
                        ImGui_SmallButton(ctx, hId);
                        int dndF = 0;
                        if (ImGui_BeginDragDropSource(ctx, &dndF)) {
                            char pl[8]; snprintf(pl, sizeof(pl), "%d", i);
                            ImGui_SetDragDropPayload(ctx, "FXL_PSORD",
                                                     pl, nullptr);
                            ImGui_Text(ctx, disp);
                            ImGui_EndDragDropSource(ctx);
                        }
                        if (ImGui_BeginDragDropTarget(ctx)) {
                            char buf[8] = {0}; int af = 0;
                            if (ImGui_AcceptDragDropPayload(ctx, "FXL_PSORD",
                                    buf, static_cast<int>(sizeof(buf)), &af)) {
                                dragFrom = std::atoi(buf); dragTo = i;
                            }
                            ImGui_EndDragDropTarget(ctx);
                        }
                        ...
                        if (foreign) {
                            ImGui_SameLine(ctx, nullptr, nullptr);
                            char rmId[40];
                            snprintf(rmId, sizeof(rmId), "x##fxl_ps_rm_%d", i);
                            if (ImGui_SmallButton(ctx, rmId)) removeIdx = i;
                        }
                    }
                    if (removeIdx >= 0) {
                    ...
                    if (!slotPushSteps_(ctrl.linkIdx).empty()) {
                        if (ImGui_SmallButton(ctx, boundDiscrete
                                ? "Reset (cycle all)##fxl_ps_reset"
                                : "Clear steps##fxl_ps_reset"))
                            setSlotPushSteps_(ctrl.linkIdx, {});
                    }
                }

                // + Add step — pick a plugin param (only ones that expose
                // discrete options), then one of its values. Works off the
                // live FX when present, else the param snapshot.
                if (ImGui_BeginMenu(ctx, "+ Add step##fxl_ps_add", nullptr)) {
                    const int pcount = editing
                        ? paramCountFor_(*editing, fx) : 0;
                    // Param search box.
                    int fFlags = 0;
                    ImGui_SetNextItemWidth(ctx, scaleW_(ctx, 200.0));
                    ImGui_InputText(ctx, "##fxl_ps_filter", g_fxlAddFilter,
                    ...
                    char curPName[96] = {0};
                    if (g_fxlAddParam >= 0 && editing)
                        paramNameFor_(*editing, fx, g_fxlAddParam,
                                      curPName, sizeof(curPName));
                    if (ImGui_BeginCombo(ctx, "##fxl_ps_param",
                    ...
                    // Value picker — discrete options of the chosen param.
                    if (g_fxlAddParam >= 0) {
                        double pStep = 1.0;
                        const int numSteps = optionCountFor(g_fxlAddParam, pStep);
                        if (numSteps >= 2) {
                            ...
                            for (int i = 0; i < numSteps; ++i) {
                                ...
                                if (fx.ok)
                                    TrackFX_FormatParamValueNormalized(fx.tr,
                                        ...
                                        vbuf, sizeof(vbuf));
                                char oid[96];
                                snprintf(oid, sizeof(oid), "%s##fxl_ps_opt_%d",
                                         vbuf[0] ? vbuf
                                                 : (i == 0 ? "OFF" : "ON"), i);
                                if (ImGui_SmallButton(ctx, oid)) {
                                    // Append to the effective list so the
                                    // bound param's options are materialised
                                    // too (don't drop them by writing only
                                    // the new macro step).
                                    std::vector<uf8::PushStep> v = view;
                         ...
                         bool softKeys, int idx)
{
    ...
    if (ImGui_BeginPopup(ctx, popId, nullptr)) {
        ...
        ImGui_TextDisabled(ctx, softKeys ? "Display name (13 chars)"
                                         : "Display name (11 chars)");
...
void SettingsScreen::drawFxLearn(ImGui_Context* ctx)
{
    ...
    // Modifier layers — the global enable flags for the held-modifier overlays
    // the editor's Layer tabs edit. Moved here from Behaviour → Keyboard
    // (2026-08-09). Collapsed like the skip list above so the schematic keeps
    // its height — the editor's panes size off the remaining content region.
    if (ImGui_CollapsingHeader(ctx, "Modifier layers###fxl_modlayers",
                               nullptr, nullptr)) {
...
void SettingsScreen::drawFavourites(ImGui_Context* ctx)
{
    ...
    if (ImGui_Checkbox(ctx, "Channel Strip: use own settings (vs copy values)", &csOwn))
        reasixty_setCsFavOwnSettings(csOwn);
    ...
    auto buildPool = [&](bool cs, std::vector<std::pair<std::string, std::string>>& out) {
        ...
        for (int s = 0; s < reasixty_favSetCount(cs); ++s)
            for (int i = 0; i < 8; ++i) {
                reasixty_favSetGet(cs, s, i, a, l);
                push(a, disp(a, l));
            }
    };

    // --- Favourite sets: CS and BC libraries, side by side -------------------
    ImGui_Spacing(ctx); ImGui_Separator(ctx);
    ImGui_Text(ctx, "Favourite sets");
    ImGui_TextDisabled(ctx, "Channel-Strip and Bus-Comp sets are separate libraries. "
                            "Edit the base Favourites or a named set; assign one CS and "
                            "one BC set per track below.");

    std::vector<std::pair<std::string, std::string>> poolCs, poolBc;
    buildPool(true, poolCs);
    buildPool(false, poolBc);

    // s_csSel / s_bcSel: -1 = base bank, 0..n-1 = a named set in that library.
    static int s_csSel = -1, s_bcSel = -1;

    // One domain's library editor: bank selector + new/dup/del + rename + 8 slots.
    // No nested table (the outer two-column table is the only table here).
    auto drawLibrary = [&](bool cs) {
        auto& pool = cs ? poolCs : poolBc;
        int&  sel  = cs ? s_csSel : s_bcSel;
        const char* dom = cs ? "cs" : "bc";
        const int setCount = reasixty_favSetCount(cs);
        if (sel >= setCount) sel = setCount - 1;
        if (sel < -1) sel = -1;
        bool isBase = (sel < 0);

        ImGui_Text(ctx, cs ? "Channel Strip sets" : "Bus Compressor sets");

        std::string selName;
        if (!isBase) reasixty_favSetName(cs, sel, selName);
        const std::string baseLabel = reasixty_favProjectBank()
                                          ? "Base (this project)" : "Base (global)";
        char id[32]; snprintf(id, sizeof(id), "##fav_sel_%s", dom);
        ImGui_SetNextItemWidth(ctx, -1.0);
        if (ImGui_BeginCombo(ctx, id, isBase ? baseLabel.c_str() : selName.c_str(), nullptr)) {
            bool selBase = isBase;
            char bid[32]; snprintf(bid, sizeof(bid), "Base Favourites##fb_%s", dom);
            if (ImGui_Selectable(ctx, bid, &selBase, nullptr, nullptr, nullptr)) sel = -1;
            for (int s = 0; s < setCount; ++s) {
                std::string nm; reasixty_favSetName(cs, s, nm);
                char item[96]; snprintf(item, sizeof(item), "%s##fs_%s_%d", nm.c_str(), dom, s);
                bool issel = (s == sel);
                if (ImGui_Selectable(ctx, item, &issel, nullptr, nullptr, nullptr)) sel = s;
            }
            ImGui_EndCombo(ctx);
        }
        char nbid[32]; snprintf(nbid, sizeof(nbid), "New##fn_%s", dom);
        if (ImGui_Button(ctx, nbid, nullptr, nullptr)) {
            char nm[32]; snprintf(nm, sizeof(nm), "Set %d", setCount + 1);
            sel = reasixty_favSetCreate(cs, nm);
        }
        if (!isBase) {
            ImGui_SameLine(ctx, nullptr, nullptr);
            char dbid[32]; snprintf(dbid, sizeof(dbid), "Dup##fd_%s", dom);
            if (ImGui_Button(ctx, dbid, nullptr, nullptr)) sel = reasixty_favSetDuplicate(cs, sel);
            ImGui_SameLine(ctx, nullptr, nullptr);
            char xbid[32]; snprintf(xbid, sizeof(xbid), "Del##fx_%s", dom);
            if (ImGui_Button(ctx, xbid, nullptr, nullptr)) {
                reasixty_favSetDelete(cs, sel);
                if (sel >= reasixty_favSetCount(cs)) sel = reasixty_favSetCount(cs) - 1;
            }
        }

        // Re-read (a delete may have shifted) + rename field for sets.
        if (sel >= reasixty_favSetCount(cs)) sel = reasixty_favSetCount(cs) - 1;
        isBase = (sel < 0);
        if (!isBase && reasixty_favSetName(cs, sel, selName)) {
            char nameBuf[64]; snprintf(nameBuf, sizeof(nameBuf), "%s", selName.c_str());
            char rid[32]; snprintf(rid, sizeof(rid), "##fav_ren_%s", dom);
            ImGui_SetNextItemWidth(ctx, -1.0);
            if (ImGui_InputTextWithHint(ctx, rid, "set name", nameBuf, sizeof(nameBuf), nullptr, nullptr))
                reasixty_favSetRename(cs, sel, nameBuf);
        }

        // 8 slots — one searchable combo per row (number prefix via SameLine).
        for (int i = 0; i < 8; ++i) {
            ...
            std::string a, l;
            if (isBase) { if (cs) baseCsFav(i, a, l); else baseBcFav(i, a, l); }
            else        reasixty_favSetGet(cs, sel, i, a, l);

            char cid[24]; snprintf(cid, sizeof(cid), "##slot_%s_%d", dom, i);
            ImGui_SetNextItemWidth(ctx, -1.0);
            ...
            if (ImGui_BeginCombo(ctx, cid, preview.c_str(), nullptr)) {
                ImGui_Dummy(ctx, kPopupW, 0.0);   // pin popup width (see action picker)
                const bool justOpened = ImGui_IsWindowAppearing(ctx);
                static char setSearch[128] = {0};   // only one combo open at a time
                ...
                auto writeSlot = [&](const char* add) {
                    ...
                    else        reasixty_favSetSet(cs, sel, i, add, add);
                };
                ...
                for (size_t k = 0; k < pool.size(); ++k) {
                    ...
                    bool issel = (pool[k].first == a);
                    if (ImGui_Selectable(ctx, it, &issel, nullptr, nullptr, nullptr))
                        writeSlot(pool[k].first.c_str());
                }
                ImGui_EndCombo(ctx);
            }
        }
    ...
    ImGui_Spacing(ctx);
    int tblFlags = 0;
    if (ImGui_BeginTable(ctx, "##fav_libs", 2, &tblFlags, nullptr, nullptr, nullptr)) {
        int    wFlag = ImGui_TableColumnFlags_WidthFixed;
        double wCol  = scaleW_(ctx, 280.0);
        ImGui_TableSetupColumn(ctx, "cs", &wFlag, &wCol, nullptr);
        ImGui_TableSetupColumn(ctx, "bc", &wFlag, &wCol, nullptr);
        ImGui_TableNextColumn(ctx); drawLibrary(true);
        ImGui_TableNextColumn(ctx); drawLibrary(false);
        ImGui_EndTable(ctx);
    }

    // --- Per-track assignment (one CS set + one BC set) ----------------------
    ImGui_Spacing(ctx); ImGui_Separator(ctx);
    ImGui_Text(ctx, "Assign sets to selected tracks");
    const int nSel = CountSelectedTracks(nullptr);
    if (nSel <= 0) {
        ImGui_TextDisabled(ctx, "Select one or more tracks to assign a CS and/or BC set.");
    } else {
        std::string names;
        ...
        // One assignment combo for a domain — common value or "(mixed)".
        auto assignCombo = [&](bool cs) {
            const char* dom = cs ? "cs" : "bc";
            std::string common; bool first = true, mixed = false;
            for (int i = 0; i < nSel; ++i) {
                MediaTrack* t = GetSelectedTrack(nullptr, i);
                if (!t) continue;
                std::string a; const bool has = reasixty_trackAssignedSet(cs, t, a);
            ...
            ImGui_Text(ctx, cs ? "CS set" : "BC set");
            ImGui_SameLine(ctx, nullptr, nullptr);
            char id[24]; snprintf(id, sizeof(id), "##fav_trk_%s", dom);
            ImGui_SetNextItemWidth(ctx, scaleW_(ctx, 240.0));
            if (ImGui_BeginCombo(ctx, id, preview, nullptr)) {
                char nid[40]; snprintf(nid, sizeof(nid), "(Base Favourites)##ftn_%s", dom);
                bool selNone = !mixed && common.empty();
                if (ImGui_Selectable(ctx, nid, &selNone, nullptr, nullptr, nullptr))
                    for (int i = 0; i < nSel; ++i)
                        if (MediaTrack* t = GetSelectedTrack(nullptr, i)) reasixty_assignTrackSet(cs, t, "");
                for (int s = 0; s < reasixty_favSetCount(cs); ++s) {
                    std::string nm; reasixty_favSetName(cs, s, nm);
                    char item[96]; snprintf(item, sizeof(item), "%s##ft_%s_%d", nm.c_str(), dom, s);
                    bool issel = (!mixed && common == nm);
                    if (ImGui_Selectable(ctx, item, &issel, nullptr, nullptr, nullptr))
                        for (int i = 0; i < nSel; ++i)
                            if (MediaTrack* t = GetSelectedTrack(nullptr, i)) reasixty_assignTrackSet(cs, t, nm.c_str());
                }
                ImGui_EndCombo(ctx);
            }
        };
