...
- **SSL preset browser** on the UF1 and the UC1. SSL's plug-ins expose no presets to the host at all, so REAPER's own preset list is empty for every one of them; Rea-Sixty reads SSL's library off disk instead, in the folders SSL files it in, and loads a preset through the plug-in's own state so the instance reports it as loaded rather than as the last one with changes. On the UC1 that is the PRESETS screen, with each half browsing its own section's instance.
...
- **Bindings system**: per-strip, transport, global, soft-keys per layer; modifier combos (incl. double-click latch on Shift); seven **Soft-Key Sets** of six banks each, 42 user-defined banks of 8 buttons, plus the 6 SSL stock banks. A set has a number and a name and is engaged by a bindable action, so it sits on any key, foot-switch or Stream Deck tile rather than only on its Quick button, and the Bindings pane shows all seven and their banks as one matrix. Each bank carries **two full sets of keys** (Plain and Shift) with their own dynamic kind, presets and LED colour, so one set can be eight fixed actions while the other is an FX bank; on the SSL banks the keys the plug-in leaves free are yours on every set, and every modifier set is yours even on the keys it fills; keyboard-macro actions that fire arbitrary key-chords into REAPER; Learn mode and right-click Copy / Paste in the Bindings editor.
