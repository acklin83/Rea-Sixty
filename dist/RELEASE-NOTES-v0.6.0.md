...
**Two builtins the picker could never show.** UF1 V-Pot push and UF1 Sends/Receives toggle were bindable, carried a description and a label, and sat in no category, and the picker drops a builtin whose category is empty: the only way onto a key was to already have it on one. They are in Hardware Modes and in Sends / Receives now, and the check that CI runs fails on any builtin the picker cannot show, which is what found them.

**Bindings.** A press on a surface selects that button in Settings, Bindings, so you no longer hunt for it in the schematic. It can be switched off under Behaviour, and the key that opens and closes Settings keeps working, or there would be no way back out from the surface. And with "Layers switch Quicks only" on, the pane shows the layer that fires. The surface keeps Layer 1's bindings there while LAYER 2 and 3 move the Quicks, and the pane answered both questions with one number, so a binding looked deleted while the key on the surface went on firing it. Reset to factory, the layer export and import and the UC1 pair read the same layer; the matrix, the engaged Quick and the slot editor stay with the Quick layer.
...
**All three surfaces name the input the keys are driving.** The UF8 and the UF1 had been putting the track's hardware input in the cell that otherwise names the plug-in; the UC1 flashed it for a second and a half after a channel change and then went back to naming the plug-in. It now stands in the colour bar and in the model field above the track name, from the same source as the other two, and it repaints when the input changes.

**And the value line belongs to the preamp**, on a hardware input, on all three. On a channel without a preamp it says so with empty flags and "--dB": the UF8 had always answered that way, the UF1 fell back to pan, which is not what you read on a mic input, and the UC1 left its last reading standing. Pointing a track from a channel with a preamp at one without used to leave the old gain on the UC1, which was the only surface that did not rebuild the line every tick. A track that is not on a hardware input at all keeps whatever its value line normally shows.
