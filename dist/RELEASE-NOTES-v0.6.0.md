...
**REC.** Changing a track's input channel with Shift and a rotation works without an RME interface. The input is REAPER's own; only preamp gain, 48V, pad and phase need TotalReaper. On a hardware input the value line belongs to the preamp on all three surfaces, and on a channel without one it says so with empty flags and "--dB". The UF8 had always answered that way; the UF1 fell back to pan, which is not what you read on a mic input, and the UC1 left its last reading standing. A track that is not on a hardware input at all keeps whatever its value line normally shows.
